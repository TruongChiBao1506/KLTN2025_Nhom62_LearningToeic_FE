...
const QuestionEditNo6To7 = ({ sectionId, questionId, retrieveQuestions, onClose }) => {
  const [question, setQuestion] = useState(null);
  const [editorText, setEditorText] = useState('');
  const [editorAnswer, setEditorAnswer] = useState('');
  const editorTextRef = useRef(null);
  const editorAnswerRef = useRef(null);
  ...
  useEffect(() => {
    const getQuestion = async () => {
      try {
        ...
        setEditorText(data.questionText || "");
        setEditorAnswer(data.suggestedAnswer || "");
        setQuestion(data);
      } catch (error) {
  ...
  const updateQuestion = async (values, { setSubmitting }) => {
    try {
      ...
      formData.append("questionText", editorText);
      formData.append("suggestedAnswer", editorAnswer);
            ...
            <div className="col">
              <div className="form-group mb-3">
                ...
                <div className="ckeditor-container">
                  <CKEditorOptimized
                    ...
                    onReady={(editor) => { editorTextRef.current = editor; }}
                    placeholder="Nhập nội dung câu hỏi..."
                    height="170px"
                  />
                </div>
                {!editorText && formik.submitCount > 0 && (
              ...
              <div className="form-group mb-3">
                ...
                <div className="ckeditor-container">
                  <CKEditorOptimized
                    data={editorAnswer}
                    onChange={setEditorAnswer}
                    onReady={(editor) => { editorAnswerRef.current = editor; }}
                    placeholder="Nhập đáp án gợi ý..."
                    height="170px"
                  />
                </div>
                {!editorAnswer && formik.submitCount > 0 && (
...
export default QuestionEditNo6To7;