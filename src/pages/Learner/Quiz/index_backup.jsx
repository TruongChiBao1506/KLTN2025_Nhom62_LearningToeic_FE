...
const Quiz = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        console.log("🚀 ~ vocabResponse isArray:", Array.isArray(vocabResponse));
        ...
        const vocabList = Array.isArray(vocabResponse) 
          ? vocabResponse.filter(vocab => {
              ...
              return vocab.word && vocab.meaning && vocab.vocabularyStatus === 1;
            })
          : [];
  ...
  const generateQuestions = (vocabList) => {
    ...
    console.log("🚀 ~ Will generate", maxQuestions, "questions from", shuffledVocabs.length, "vocabularies");
    ...
    shuffledVocabs.slice(0, maxQuestions).forEach((vocab, index) => {
      ...
      const otherVocabs = vocabList.filter(v => 
        v._id !== vocab._id && 
        v.meaning && 
      ...
      console.log(`🚀 ~ Found ${otherVocabs.length} other vocabs for wrong answers`);
      
      if (otherVocabs.length < 3) {
        console.warn(`Not enough wrong answers for vocab "${vocab.word}" (found ${otherVocabs.length}, need 3)`);
        return;
      }
      ...
      const options = [vocab.meaning, ...wrongAnswers]
        .sort(() => 0.5 - Math.random());
      ...
      console.log(`🚀 ~ Generated question ${questions.length + 1}:`, questionData);
              ...
              <div className="card results-card fade-in">
                <div className="card-body text-center p-5">
                  ...
                  <div className="results-icon mb-4">
                    <div className={percentage >= 70 ? "success-checkmark" : "error-cross"}></div>
                  </div>

                  {/* Score Ring */}
                  <div className="score-ring mb-4">
                    <svg viewBox="0 0 150 150">
                      <circle className="bg" stroke-dasharray="408.4" stroke-dashoffset="0" />
                      <circle 
                        className="progress" 
                        stroke-dasharray="408.4" 
                        stroke-dashoffset={408.4 - (408.4 * percentage) / 100}
                      />
                    </svg>
                  ...
                  <h2 className="mb-3 text-gradient">
                    {percentage >= 90 ? "🎉 Xuất sắc!" : 
                     ...
                     percentage >= 50 ? "📚 Cần cố gắng thêm!" : "💪 Hãy thử lại!"}
                  </h2>
                  ...
                  <div className="d-flex flex-wrap gap-3 justify-content-center mb-4">
                    ...
                    <Link to={`/learner/topic/${topicId}`} className="btn btn-secondary btn-lg">
                      <FontAwesomeIcon icon={faArrowLeft} className="me-2" />
                      Quay lại
                    </Link>
                    <Link to={`/learner/topic/${topicId}/flashcards`} className="btn btn-warning btn-lg">
                          ...
                          <div className="d-flex align-items-center justify-content-between">
                            <div className="flex-grow-1">
                              <div className="review-word mb-2">
                                ...
                                {questions[index]?.ipa && (
                                  <span className="ipa ms-2">{questions[index].ipa}</span>
                                )}
                              </div>
                              ...
                              {!answer?.isCorrect && (
                                <div className="review-correct">
                                  <span className="fw-medium">Đáp án đúng: </span>
      ...
      <div className="container-fluid quiz-intro">
        <div className="row justify-content-center">
          <div className="col-md-8">
            <div className="card intro-card scale-in">
              <div className="card-body text-center p-5">
                <div className="intro-icon mb-4 floating">
                  <FontAwesomeIcon icon={faQuestionCircle} className="fs-1" />
                </div>
                <h2 className="mb-3 text-gradient">Kiểm tra từ vựng</h2>
                <h4 className="text-primary mb-4">{topic.topicName || 'Chủ đề không xác định'}</h4>
                
                <div className="quiz-info mb-5">
                  <div className="row text-center">
                    <div className="col-md-4">
                      <div className="info-item">
                        <FontAwesomeIcon icon={faQuestionCircle} className="fs-3 text-primary mb-2" />
                        <h5>{questions.length}</h5>
                    ...
                    <div className="col-md-4">
                      <div className="info-item">
                        <FontAwesomeIcon icon={faClock} className="fs-3 text-warning mb-2" />
                    ...
                    <div className="col-md-4">
                      <div className="info-item">
                        <FontAwesomeIcon icon={faStar} className="fs-3 text-success mb-2" />
                        <h5>70%</h5>
                ...
                <div className="d-flex gap-3 justify-content-center">
                  <button onClick={startQuiz} className="btn btn-success btn-lg pulse">
          ...
          <div className="quiz-header">
            <div className="d-flex justify-content-between align-items-center">
              ...
              <div className="quiz-timer">
                ...
                <span className={`fs-6 fw-bold ${timeLeft <= 30 ? 'text-danger pulse' : 'text-dark'}`}>
                  {formatTime(timeLeft)}
                </span>
              </div>
            </div>
          ...
          <div className="card question-card fade-in">
            <div className="card-body p-4">
              <div className="question-section text-center mb-4">
                <h3 className="question-title">Nghĩa của từ này là gì?</h3>
                <div className="word-display">
                  <h1 className="word floating">{question.word || 'Unknown'}</h1>
                  {question.ipa && (
                    <div className="pronunciation">
                      <span className="ipa">{question.ipa}</span>
                      <button 
                ...
                {question.exampleSentence && question.exampleSentence.trim() && (
                  <div className="example-hint mt-2">
                    <small className="text-muted">
                      💡 Ví dụ: <em>{question.exampleSentence}</em>
                    </small>
                  </div>
                )}
              </div>
