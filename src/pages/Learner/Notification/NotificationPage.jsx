...
const NotificationPage = () => {
  ...
  const notifications = getNotificationsByRole(allNotifications, 'learner');
  
  // Get learner user info
  const learnerUserData = JSON.parse(localStorage.getItem('user') || '{}');
  ...
  const getNotificationIcon = (type) => {
    const iconMap = {
      ...
      'milestone': faStar,
      'course_complete': faGraduationCap,
      'system': faCog,
  ...
  useEffect(() => {
    document.title = "Thông báo | Learner Dashboard";
  }, []);
        ...
        <div className="notif-header-bar">
          ...
          {notifications.length > 0 && notifications.some(n => !n.isRead) && (
            <button
              ...
              onClick={() => dispatch(markAllAsRead(learnerUserData._id || learnerUserData.id))}
            >
