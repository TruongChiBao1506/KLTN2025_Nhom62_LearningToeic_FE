...
const StudyTimer = () => {
  ...
  useEffect(() => {
    ...
    const savedSessions = JSON.parse(
      localStorage.getItem("studyTimer_sessions") || "[]"
    );
    const savedStats = JSON.parse(
      localStorage.getItem("studyTimer_stats") || "{}"
    );
    const savedSettings = JSON.parse(
      localStorage.getItem("studyTimer_settings") || "{}"
    );
  ...
  const handleTimerComplete = useCallback(() => {
    ...
    const newSession = {
      ...
      duration: isBreak
        ? sessions.length % settings.sessionsBeforeLongBreak === 0
          ? settings.longBreak
          : settings.shortBreak
        : settings.workTime,
    ...
    setStats((prevStats) => {
      ...
      if (!isBreak) {
        ...
        const yesterdaySessions = updatedSessions.filter(
          (s) => s.date === yesterday && s.type === "work"
        );

        if (todaySessions.length === 1) {
          newStats.streak =
            yesterdaySessions.length > 0 ? newStats.streak + 1 : 1;
        }
      }
      ...
      <div className="timer-content">
        <div className="timer-main">
          <div className="timer-display">
            <div
              className={`timer-circle ${isBreak ? "break" : "work"} ${
                isRunning ? "running" : ""
              }`}
            >
              <div
        ...
        <div className="timer-sidebar">
          <div className="stats-section">
            ...
            <div className="stats-grid">
              ...
              <div className="stat-item">
                <span className="stat-number">
                  {Math.round(stats.weekMinutes / 7)}
                </span>
          ...
          <div className="sessions-section">
            ...
            <div className="sessions-list">
              {todaySessions.slice(-5).map((session) => (
                <div
                  key={session.id}
                  className={`session-item ${session.type}`}
                >
                  <div className="session-info">
                    <span className="session-task">
                      {session.task ||
                        (session.type === "work" ? "Study Session" : "Break")}
                    </span>
              ...
              {todaySessions.length === 0 && (
                <div className="no-sessions">Chưa có session nào hôm nay</div>
              )}
            </div>
          </div>
