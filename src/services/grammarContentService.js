...
class GrammarContentService {
    ...
    async getGrammarContentsByGrammar(grammarId) {
        try {
            ...
            if (error.response?.status === 404) {
                console.log('📝 No grammar contents found for this grammar - returning empty array');
    ...
    async getEnableGrammarContentsByGrammar(grammarId) {
        try {
            ...
            // Handle 404 as empty data instead of error
            if (error.response?.status === 404) {
                console.log('📝 No enabled grammar contents found for this grammar - returning empty array');
                return []; // Return empty array instead of throwing error
            }
    ...
    async exportByGrammar(grammarId) {
        try {
            ...
            const response = await axiosClient.get(`${this.baseUrl}/export/${grammarId}`, {
                responseType: 'blob'
            });
    ...
    async importTemplate(file, grammarId) {
        try {
            console.log('📤 Importing grammar contents from file for grammar:', grammarId);
    ...
    async bulkImport(grammarId, data) {
        try {
            ...
            const response = await axiosClient.post(`${this.baseUrl}/bulk-import/${grammarId}`, data, {
                headers: {
                    'Content-Type': 'multipart/form-data'
                }
            });
    ...
    async bulkUpdateStatus(grammarContentIds, newStatus) {
        try {
            console.log('🔄 Bulk updating status for contents:', grammarContentIds, newStatus);
    ...
    async duplicate(grammarContentId, targetGrammarId) {
        try {
            console.log('📋 Duplicating grammar content:', grammarContentId, 'to grammar:', targetGrammarId);
