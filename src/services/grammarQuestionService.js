...
class GrammarQuestionService {
    ...
    async getGrammarQuestionsByGrammar(grammarId) {
        try {
            ...
            if (error.response?.status === 404) {
                console.log('📝 No grammar questions found for this grammar - returning empty array');
    ...
    async getEnableGrammarQuestionsByGrammar(grammarId) {
        try {
            ...
            // Handle 404 as empty data instead of error
            if (error.response?.status === 404) {
                console.log('📝 No enabled grammar questions found for this grammar - returning empty array');
                return []; // Return empty array instead of throwing error
            }
    ...
    async updateStatus(questionId, newStatus) {
        try {
            console.log('🔄 Updating grammar question status:', questionId, newStatus);
    ...
    async importTemplate(file, grammarId) {
        try {
            console.log('📤 Importing grammar questions from file for grammar:', grammarId);
    ...
    async bulkUpdateStatus(grammarQuestionIds, newStatus) {
        try {
            console.log('🔄 Bulk updating status for questions:', grammarQuestionIds, newStatus);
