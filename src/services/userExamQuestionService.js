...
class UserExamQuestionService {
    ...
    async getQuestionsByUserExamId(userExamId) {
        const response = await axiosClient.get(`${this.baseUrl}/by-user-exam/${userExamId}`);
        return response;
    }

    async getQuestionsByUserExamIdGroupedByType(userExamId) {
        const response = await axiosClient.get(`${this.baseUrl}/by-user-exam/${userExamId}/grouped`);
        return response;
    }

    async getAccuracyByQuestionTypeForUser(questionPart, userId) {
        const response = await axiosClient.get(`${this.baseUrl}/accuracy-by-part/${questionPart}/user/${userId}`);
        return response;
    }
}
