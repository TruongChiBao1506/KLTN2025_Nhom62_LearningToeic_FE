...
const TestPart7Triple = ({
  ...
  const getCorrectCount = useMemo(() => {
    return questions.filter((q) => q.answered && q.selectedOption === q.correctOption).length;
  }, [questions]);
  ...
  return (
    <>
                        ...
                        <div className="audio-image-container mt-5">
                          {shouldDisplayGroupContent(groupQuestions) && (
                            <div className="audio-container mb-2">
                              <div dangerouslySetInnerHTML={{ __html: groupQuestions[0].questionGroup.groupPassage }} />
                            </div>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="scrollable-container">
                        {groupQuestions.map((question, index) => (
                          <div key={index} className="ms-3">
                            <div id={`question-${groupId}-${index}`} className="question">
                              <span className="badge text-bg-secondary mb-2" style={{ fontSize: '14px' }}>
                              ...
                              <div className="row">
                                <ul className="mt-5">
                                  ...
                                  {getOptions(question).map((option, optionIndex) => (
                                    <li key={optionIndex} className={getOptionClass(question, option)}>
                                      <label className="form-check-label">
                                        ...
                                        {question.isGraded && option === question.correctOption && (
                                          <div className="result-icon">
                                            <i className="fas fa-check" style={{ color: 'green' }}></i>
                                          </div>
                                        )}
                                        {question.isGraded && option === question.selectedOption && option !== question.correctOption && (
                                          <div className="result-icon">
                                ...
                                {question.isGraded && (
                                  <div className="feedback-section">
                                    <button 
                                      onClick={() => toggleExplanation(calculateQuestionNumber(groupId, index))}
                                      className="btn btn-link btn-sm mt-2 link-offset-3"
                                    >
                                      {showExplanation[calculateQuestionNumber(groupId, index)] ? 'Ẩn giải thích' : 'Hiện giải thích'}
                                    </button>
                                    {showExplanation[calculateQuestionNumber(groupId, index)] && (
                                      <div className="transcript">
                                        <div className="explanation-original">
                                          <div dangerouslySetInnerHTML={{ __html: question.questionExplanation }} />
                                        </div>
                                        {question.translatedExplanation && (
                                          <div className="transcript-translation">
                                            <span className="badge text-bg-success">Bản dịch</span>
                                            <div dangerouslySetInnerHTML={{ __html: question.translatedExplanation }} />
                                          </div>
                                        )}
                                      </div>
                                    )}
      ...
      <div className="col-lg-4 col-md-4 col-sm-4 text-decoration-none border-0">
        <div className="card specific-card border-0" style={{ position: 'sticky', top: '95px', zIndex: 1 }}>
          <div className="card-body border-0">
            <div className="question-list-section">
              <h5 className="fw-normal fs-5 text-center">Bảng câu hỏi</h5>
              <div className="question-buttons mb-5 mx-2 lesson-content" style={{ display: 'flex', flexWrap: 'wrap' }}>
                {Object.entries(groupedQuestions).map(([groupId, groupQuestions]) => (
              ...
              <div className="score">
                <i className="fas fa-square" style={{ color: 'green' }}></i> {getCorrectCount}/{questions.length}
                <i className="fas fa-square ms-2" style={{ color: 'red' }}></i> {getIncorrectCount}/{questions.length}
              </div>
