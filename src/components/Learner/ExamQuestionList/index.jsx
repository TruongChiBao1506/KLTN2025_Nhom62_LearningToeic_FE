...
const ExamQuestionList = ({ 
      ...
      try {
        ...
        if (selected && selected.length > 0) {
          const updatedQuestions = examQuestions.map(q => {
            const savedQuestion = selected.find(sq => sq.examQuestionId === q.examQuestionId);
  ...
  useEffect(() => {
    if (!hasSubmitted && userExamId) {
      ...
      localStorage.setItem(`exam_${userExamId}_state`, JSON.stringify(stateToSave));
    }
  ...
  useEffect(() => {
    const handleBeforeUnload = (event) => {
      if (!hasSubmitted) {
        const message = "Bạn có chắc chắn muốn rời khỏi trang? Dữ liệu làm bài của bạn có thể bị mất!";
  ...
  const handleToggleFlag = (examQuestion) => {
    ...
    const index = newFlaggedQuestions.findIndex(q => q.examQuestionId === examQuestion.examQuestionId);
  ...
  const isFlagged = (examQuestion) => {
    return flaggedQuestions.some(q => q.examQuestionId === examQuestion.examQuestionId);
  };
  ...
  const scrollToQuestion = (part, index) => {
    ...
    if (examQuestionElement) {
      ...
      const question = examQuestions.find(q => q.questionPart === part && q.orderNumber === index);
  ...
  const navigateToNextQuestion = (currentPart, currentNumber) => {
    ...
    const currentIndex = currentPartQuestions.findIndex(q => q.orderNumber === currentNumber);
    
    if (currentIndex < currentPartQuestions.length - 1) {
      // Còn câu tiếp theo trong cùng phần
      scrollToQuestion(currentPart, currentPartQuestions[currentIndex + 1].orderNumber);
      return;
    }
    ...
    if (currentPartIndex < parts.length - 1) {
      ...
      if (groupedQuestionsByPart[nextPart]?.length > 0) {
        scrollToQuestion(nextPart, groupedQuestionsByPart[nextPart][0].orderNumber);
  ...
  const navigateToPrevQuestion = (currentPart, currentNumber) => {
    ...
    if (currentPartIndex > 0) {
      ...
      if (prevPartQuestions.length > 0) {
        scrollToQuestion(prevPart, prevPartQuestions[prevPartQuestions.length - 1].orderNumber);
  ...
  const getCorrectCount = () => {
    return localExamQuestions.filter((q) => q.answered && q.selectedOption === q.correctOption).length;
  };
  ...
  const InstructionModal = () => {
    ...
    switch (modal.part) {
      case 'PART1':
        instructionText = 'Nhìn hình ảnh và nghe câu. Chọn câu mô tả chính xác nhất hình ảnh.';
        break;
      case 'PART2':
        instructionText = 'Nghe câu hỏi và ba câu trả lời. Chọn câu trả lời phù hợp nhất với câu hỏi.';
        break;
      case 'PART3':
        instructionText = 'Nghe đoạn hội thoại. Sau đó đọc từng câu hỏi và chọn câu trả lời tốt nhất.';
        break;
      case 'PART4':
        instructionText = 'Nghe đoạn nói chuyện. Sau đó đọc từng câu hỏi và chọn câu trả lời tốt nhất.';
        break;
      case 'PART5':
        instructionText = 'Chọn từ hoàn thành câu một cách tốt nhất.';
        break;
      case 'PART6':
        instructionText = 'Chọn từ hoặc cụm từ hoàn thành chỗ trống một cách tốt nhất.\nChủ đề: Thông báo, thư, email, quảng cáo.';
        break;
      case 'PART7':
      ...
      <div className="col-lg col-md col-sm">
        <div className="card specific-card border-0 shadow-lg mb-4">
          <div className="card-body">
            ...
            {/* Thông báo lưu ý */}
            <div className="alert alert-primary alert-dismissible fade show" role="alert" style={{ fontSize: '16px' }}>
              <p className="text-warning"><i className="fa-solid fa-triangle-exclamation"></i> Lưu ý: </p>
            ...
            <div className="d-flex justify-content-end fst-italic">
              ...
              >
                <i style={{ fontSize: '30px' }} className="fa-regular fa-circle-question mx-2"></i>
              </button>
            </div>
            ...
            <div className="row">
              {Object.entries(groupedQuestions).map(([groupId, questions]) => (
                <div className="col-sm-12" key={groupId}>
                  ...
                  {shouldDisplayAudio(questions[0]) && !questions[0].hasListened && (
                    <div className="audio-container mt-5 mb-2">
                      ...
                      >
                        <source src={getAudioUrl(questions[0].questionAudio)} type="audio/mpeg" />
                  ...
                  <div className="row">
                    ...
                    {(shouldDisplayImage(questions[0]) || shouldDisplayPassage(questions[0])) && (
                      <div 
                        className={`
                          ${(shouldDisplayImage(questions[0]) || (shouldDisplayPassage(questions[0]) && questions.length >= 2)) ? 'col-md-6' : 'col-md-12'}
                          ${(shouldDisplayImage(questions[0]) || (shouldDisplayPassage(questions[0]) && questions.length >= 2)) ? 'bg-light rounded' : ''}
                          ${(shouldDisplayPassage(questions[0]) && questions.length >= 4) ? 'scrollable-container' : ''}
                        `}
                      >
                        ...
                        {shouldDisplayPassage(questions[0]) && (
                          <div className="audio-image-container mt-5">
                            <div className="audio-container mb-2">
                              <div dangerouslySetInnerHTML={{ __html: questions[0].questionPassage }}></div>
                            </div>
                          </div>
                        )}
                      </div>
                    ...
                    <div className="col-md">
                      <div className={`${shouldDisplayPassage(questions[0]) && questions.length >= 4 ? 'scrollable-container' : ''}`}>
                        {questions.map((examQuestion, index) => (                          <div 
                            key={examQuestion.examQuestionId} 
                            id={`examQuestion-part-${selectedPart}-${examQuestion.orderNumber}`}
                            className={`ms-3 question ${isQuestionViewed(examQuestion) ? 'viewed-question' : ''}`}
                            onClick={() => markQuestionAsViewed && markQuestionAsViewed(examQuestion.examQuestionId)}
                          >
                            <div className="row">
                              {!examQuestion.isGraded && (
                                <div className="text-end mt-2">
                                  <button 
                                    onClick={() => handleToggleFlag(examQuestion)} 
                              ...
                              <ul>
                                ...
                                <span className="ms-1">{examQuestion.questionContent}</span>
                                {getOptions(examQuestion).map((option, optionIndex) => (
                                  <li 
                                    key={optionIndex} 
                                    className={option === examQuestion.selectedOption ? 'highlight-row' : ''}
                                    onClick={() => !examQuestion.isGraded && handleSelectOption(examQuestion, option)}
                                  >
                                    <label className="form-check-label">
                                      <input 
                                        ...
                                        type="radio" 
                                        value={option} 
                                        checked={examQuestion.selectedOption === option}
                                        disabled={examQuestion.isGraded}
                                        onChange={() => !examQuestion.isGraded && handleSelectOption(examQuestion, option)}
                                        name={`flexRadioDefault-${examQuestion.examQuestionId}`} 
                                      />
                                      {option}

                                      {examQuestion.isGraded && option === examQuestion.correctOption && (
                                        <div className="result-icon">
                                          <i className="fas fa-check" style={{ color: 'green' }}></i>
                                        </div>
                                      )}
                                      {examQuestion.isGraded && option === examQuestion.selectedOption && option !== examQuestion.correctOption && (
                                        <div className="result-icon">
                                ...
                                {examQuestion.isGraded && examQuestion.questionExplanation && (
                                  <div className="feedback-section">
                                    <button 
                                      onClick={() => toggleExplanation(index)}
                                      className="btn btn-link btn-sm mt-2 link-offset-3"
                                    >
                                      {showExplanation[index] ? 'Ẩn giải thích' : 'Xem giải thích'}
                                    </button>
                                    {showExplanation[index] && (
                                      <div 
                                        className="explanation"
                                        dangerouslySetInnerHTML={{ __html: examQuestion.questionExplanation }}
                                      />
                                    )}
                                  </div>
                                )}
                            ...
                            {questions.length > 0 && !hasSubmitted && (
                              <div className="question-navigation mt-3">
                                <button 
                                  ...
                                  onClick={() => navigateToPrevQuestion(selectedPart, questions[0].orderNumber)}
                                >
                                  <i className="fas fa-arrow-left me-1"></i> Câu trước
                                </button>
                                <button 
                                  className="btn btn-sm btn-outline-secondary"
                                  onClick={() => navigateToNextQuestion(selectedPart, questions[questions.length-1].orderNumber)}
                                >
                        ...
                        {/* Hiển thị TranScript */}
                        {isGroupAnswered(questions) && questions[0].questionScript && (
                          <>
                            <button 
                              onClick={() => toggleGroupScript(groupId)} 
                              className="btn btn-link btn-sm link-offset-3"
                            >
                              {showGroupScript[groupId] ? 'Ẩn TranScript' : 'Xem TranScript'}
                            </button>
                            {showGroupScript[groupId] && (
                              <div 
                                className="transcript mb-2"
                                dangerouslySetInnerHTML={{ __html: questions[0].questionScript }}
                              />
                            )}
                          </>
                        )}
        ...
        <div className="card specific-card border-0 shadow" style={{ position: 'sticky', top: '95px', zIndex: 1 }}>
          <div className="card-body border-0">
            <div className="countdown-section mb-2">
              <p>
                ...
                <span style={{ color: 'green', fontSize: '24px' }}>{formatTime(countdown)}</span>
              </p>
            </div>
            <h5 className="fw-normal fs-6 text-center bg-info text-white p-2 border rounded-4">Bảng câu hỏi</h5>
            <div className="question-list-section lesson-content">
              {parts.map((part) => (
                <div key={part}>
                  ...
                  > 
                    {selectedPart === part && <i className="fa-solid fa-chevron-right me-2"></i>}
                    Phần {part.replace('PART', '')}
                  </h6>

                  <div className="question-buttons mb-2 mx-2" style={{ display: 'flex', flexWrap: 'wrap' }}>
                    {groupedQuestionsByPart[part]?.map((examQuestion) => {
                      ...
                      if (examQuestion.selectedOption) {
                        if (examQuestion.isGraded) {
                          if (examQuestion.selectedOption === examQuestion.correctOption) {
                      ...
                      return (
                        <button 
                          key={examQuestion.examQuestionId}
                          onClick={() => scrollToQuestion(part, examQuestion.orderNumber)}
                          className={buttonClassName}
                          style={buttonStyle}
                        >
                          {!examQuestion.isGraded && isFlagged(examQuestion) && (
          ...
          <div className="row mt-3">
            <div className="d-flex justify-content-center">
              {hasSubmitted ? (
                <div className="score">
                  <i className="fas fa-square" style={{ color: 'green' }}></i> {getCorrectCount()}/{localExamQuestions.length}
                  <i className="fas fa-square ms-2" style={{ color: 'red' }}></i> {getIncorrectCount()}/{localExamQuestions.length}
                </div>
              ) : (
          ...
          {hasSubmitted && (
            ...
            >
              Đi tới trang đánh giá kết quả <i className="fa-solid fa-right-from-bracket ms-2"></i>
            </Link>
          )}
        </div>
      </div>
