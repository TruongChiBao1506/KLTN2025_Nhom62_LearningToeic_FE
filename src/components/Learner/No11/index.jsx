...
const No11 = ({ testId }) => {
    ...
    // Reading and countdown logic removed; No8To10 style is used
    ...
    const retrieveQuestions = useCallback(async () => {
        try {
            ...
            // Remove reading/countdown arrays
                ...
                recorder.addEventListener('stop', () => {
                    try {
                        ...
                        setRecordedAudios(prev => {
                            ...
                            if (assignedIndex >= 0) {
                                if (newAudios[assignedIndex]) {
                                    try { URL.revokeObjectURL(newAudios[assignedIndex]); } catch (e) { /* ignore */ }
                                }
                                newAudios[assignedIndex] = URL.createObjectURL(blob);
                            }
                            return newAudios;
                        });
                        console.log('[No11] Recorded audio created for index', assignedIndex, 'blob size', blob.size);
                    } catch (e) {
    ...
    // reading/tts logic removed per No8To10 behavior
    ...
    const stopRecording = (index) => {
        ...
        // stopRecordingCountdown removed - no countdowns used
        // only stop recorder; onstop will set isRecording to null and set recordedAudios
        if (mediaRecorderRef.current && mediaRecorderRef.current.state === 'recording') {
            try { mediaRecorderRef.current.stop(); } catch (e) { /* ignore */ }
        }
        // Defer stop tracks until 'stop' event has created blob
        setTimeout(() => {
    ...
    // countdown logic removed per No8To10 behavior
            ...
            speechRecognitionRef.current.addEventListener('result', (event) => {
                ...
                if (transcript !== currentTranscript.current) {
                    setRecordedText(prev => {
                        const newText = [...prev];
                        const idx = recordingIndexRef.current >= 0 ? recordingIndexRef.current : currentIndex;
                        newText[idx] = transcript;
                        return newText;
                    });
    ...
    const refreshAllQuestions = () => {
        // if currently recording, stop it before refreshing
        if (isRecording[currentIndex] === true) {
            try { stopRecording(currentIndex); } catch (e) { /* ignore */ }
        }
        setCurrentIndex(0);
        // clearInterval removed (no countdowns)
        retrieveQuestions();
        // ensure we trigger incrementParticipants when re-starting
        handleStartTest();
    };
                ...
                <div className="col-md-12">
                    <div className="card specific-card mt-3" style={{ minHeight: '500px', display: 'flex', flexDirection: 'column' }}>
                        <div className="card-body" style={{ flex: 1 }}>
                            ...
                            <h2 className="text-center my-3">Speaking: Bày tỏ ý kiến</h2>
                            ...
                            {isReadyToTest && (
                                <div>
                                    ...
                                    <div className="word-item">
                                        <div className="mb-5">
                                            {/* Removed countdown and reading UI - using No8To10 logic */}
