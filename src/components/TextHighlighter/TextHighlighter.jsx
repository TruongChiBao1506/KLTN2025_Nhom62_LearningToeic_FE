...
import { Palette, Languages, X, Plus, MessageSquare, Check } from 'lucide-react';
...
const TextHighlighter = ({ children, containerId }) => {
  ...
  const highlightText = () => {
    ...
    try {
      ...
      message.error('Could not highlight text. Selections that span multiple elements are not supported.');
    }
  };
  ...
  const translateText = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://translate.googleapis.com/translate_a/single?client=gtx&sl=auto&tl=vi&dt=t&q=${encodeURIComponent(selectedText)}`);
            ...
            <Space wrap>
              {HIGHLIGHT_COLORS.map((color) => (
                <Tooltip title={color.name} key={color.value}>
                  <div 
                    className={`toeic-color-swatch ${currentColor === color.value ? 'active' : ''}`}
  ...
  return (
    <div ref={containerRef} className="toeic-text-highlighter-container">
      ...
      >
        <div className="toeic-highlightable-content">
          {children}
        </div>
      </Popover>
    </div>
