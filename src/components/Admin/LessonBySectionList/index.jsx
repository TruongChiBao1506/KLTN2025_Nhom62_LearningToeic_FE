...
const LessonBySectionList = ({ lessons = [], sectionId, retrieveLessons }) => {
    ...
    // Helper: Normalize creator id in lesson object
    const getCreatorId = (lesson) => {
        if (!lesson) return null;
        const c = lesson.createdBy || null;
        if (!c) return null;
    ...
    const filteredLessons = useMemo(() => {
        ...
        // Start from a shallow copy of the lessons list
        let base = lessons.slice();
        // If we have a current user id and user is not admin, filter to only lessons created by them
        if (effectiveUserId && role !== 'admin') {
            base = base.filter((l) => getCreatorId(l) === effectiveUserId);
        }
    ...
    }, [lessons, searchText, effectiveUserId, role]);
