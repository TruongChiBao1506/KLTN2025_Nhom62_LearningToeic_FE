...
const Part2QuestionList = ({
    ...
    useEffect(() => {
        setIsSubmitEnabled(selectedQuestions.length === 25);
    }, [selectedQuestions]);

    const handleCheckboxChange = (questionId) => {
        if (selectedQuestions.includes(questionId)) {
            setSelectedQuestions(selectedQuestions.filter(id => id !== questionId));
        } else if (selectedQuestions.length < 25) {
    ...
    const submitQuestions = async () => {
        if (selectedQuestions.length < 25) {
            Swal.fire({
                icon: "error",
                title: "Lỗi",
                text: "Vui lòng chọn đủ 25 câu hỏi trước khi submit.",
            });
            return;
        }
        try {
            await TestService.addOrUpdateQuestionToTest(testId, selectedQuestions);
            toast.success("Chỉ định câu hỏi Phần 2 thành công", { autoClose: 1000 });
        } catch (error) {
            toast.error("Chỉ định câu hỏi Phần 2 thất bại", { autoClose: 1000 });
        }
        setSelectedQuestions([]);
            ...
            <table className="table text-center table-hover shadow">
                <thead className="shadow">
                    <tr className="align-middle">
                        ...
                        <th>SELECTED {selectedQuestions.length}/25</th>
                        <th>No.</th>
                        <th>A</th>
                        <th>B</th>
                        <th>C</th>
...
export default Part2QuestionList;