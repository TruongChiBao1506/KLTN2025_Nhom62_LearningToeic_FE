...
const TableSectionNo1To5 = ({ paginatedQuestions, currentPage, ITEMS_PER_PAGE, getImageUrl, sectionId, testId, retrieveQuestions }) => {
    ...
    const submitQuestions = async () => {
        if (selectedQuestions.length < 5) {
            Swal.fire({
                icon: 'error',
                title: 'Lỗi',
                text: 'Vui lòng chọn đủ 5 câu hỏi trước khi submit.',
            });
            return;
        }
        try {
            ...
            toast.success('Chỉ định câu hỏi Writing (1-5) thành công ', {
                autoClose: 1000
            });
        } catch (error) {
            console.log(error);
            toast.error('Chỉ định câu hỏi Writing (1-5) thất bại ', {
                autoClose: 1000
            });
        }
    ...
    const handleCheckboxChange = (questionId) => {
        ...
        setIsSubmitEnabled(selectedQuestions.length >= 5);
    };
            ...
            <table className="table text-center table-hover shadow">
                <thead className="shadow">
                    <tr className="align-middle">
                        ...
                        <th>SELECTED {selectedQuestions.length}/5</th>
                        <th>No.</th>
                        <th>IMAGE</th>
                        <th>CONTENT</th>
                        ...
                        <tr key={question.questionId} className="table-row shadow-on-hover align-middle">
                            ...
                            <td>
                                <input
                                    ...
                                    disabled={selectedQuestions.length >= 5 && !selectedQuestions.includes(question._id)}
                                />
                            </td>
...
export default TableSectionNo1To5;