...
const TableSectionNo8To10 = ({ paginatedQuestions, currentPage, ITEMS_PER_PAGE, getImageUrl, sectionId, testId, retrieveQuestions }) => {
    ...
    const submitQuestions = async () => {
        ...
        try {
            ...
            toast.success('Chỉ định câu hỏi Phần 2 thành công ', {
                autoClose: 1000
            });
        } catch (error) {
            console.log('❌ API call failed, error:', error);
            toast.error('Chỉ định câu hỏi Phần 2 thất bại ', {
                autoClose: 1000
            });
        }
                            ...
                            <tr key={question._id} className="table-row shadow-on-hover align-middle">
                                ...
                                {index === 0 && (
                                    <td className="question-image" rowSpan={groupedQuestions.length}>
                                        <img src={getImageUrl(question.questionGroup.groupImage) || ''} style={{ width: '100px' }} alt="Group" />
                                    </td>
                                )}
                                {index === 0 && (
                                    <td rowSpan={groupedQuestions.length}>
                                        <div dangerouslySetInnerHTML={{ __html: question.questionGroup.groupText }} />
                                    </td>
                                )}
...
export default TableSectionNo8To10;